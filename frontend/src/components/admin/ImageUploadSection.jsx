export default function ImageUploadSection({
  ...
  return (
    <section className="rounded-xl border border-gray-200 bg-white p-6 space-y-4">
      ...
      <h2 className="text-base font-bold text-jardinerie-text pt-2">
        Image secondaire <span className="text-sm font-normal text-gray-400">(optionnelle)</span>
      </h2>
      <div className="flex flex-col sm:flex-row gap-6 items-start">
        <div className="shrink-0">
          <img
            src={secondaryImagePreview || placeholderImg}
        ...
        <div className="flex-1 space-y-3">
          ...
          <input
            ...
            className="w-full text-sm text-gray-500 file:mr-4 file:rounded-full file:border-0 file:bg-jardinerie-bg file:px-4 file:py-2 file:text-sm file:font-medium file:text-jardinerie-primary hover:file:bg-jardinerie-primary hover:file:text-white focus:outline-none"
          />
          {secondaryUploading && <p className="text-xs text-jardinerie-primary animate-pulse">Upload en cours...</p>}
        </div>
      </div>
    </section>
  );
